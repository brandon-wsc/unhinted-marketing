...
| Session — split | Desktop 1440: `empty` (landing + recommended questions), **`recommend`** (ADR 0030 `angle_gate` park — bundled AnglePickCard; preview pane empty until draft), `preview` (chat + action trail + interrupt card with late-switch hint; preview pane with IG mock, revision/dirty badges, Confirm gate with serious copy); `generating` (same desk, interrupt **出圖** becomes spinner + 「出緊圖…」, format chips disabled); composer card stacks textarea then send/stop icon row (not overlaid on the textarea corner); queue tucks above the card. **Real publish (ADR 0022):** `published` (success receipt + 「喺 Instagram 睇」; edit row hidden; badge 已出) and `failed` (destructive receipt + 「再確認」; session stays unconfirmed) |
...
**Preview while image-parked:** if the open revision has no ready primary image and the session is `awaiting_image_ok`, the right pane is a 待出圖 board (caption + plan, Apply / 確認出街 disabled) — not the IG mock with a grey image slot. Execute in flight shows a spinner. See the UI note on [ADR 0004](../adr/0004-stop-discard-and-image-resume.md). The pending revision itself lands with PR #87.
