...
| System | Platform admin (`/system`), not tenant settings. Desktop 1440: **LLM 呼叫 · list** (filters + 9-col table + pager) and **· detail** (master-detail: condensed list, selected row `bg-accent`; inline detail card, no overlay — meta grid 時間/狀態/耗時/模型/來源 ｜ TTFT/Temperature/估算成本/類型/標記, token stats incl. Cache, Turn / Session cross-links, prompt blocks on `bg-secondary`). **Node steps · detail** (same split; output pre + LLM-call list). **Research · detail** (same split; condensed turns table 時間/Semantic/rule_pass/Queries/Hits; Turn 明細 card = fact rows incl. turn id, queries list, signal cards with `q:` / url / excerpt; source chip `bg-secondary`). **Session Trace** (summary / messages / revisions + grounding / turns; selected revision `border-voice-border` + `bg-accent`). **Instance** (read-only viewer: `info` alert, fields on `bg-secondary`). Status: ok = `success-soft`, error = `destructive-soft`, cancelled / empty / fallback = voice wash. **Mobile 390:** list full-width; detail is a right sheet over a scrim (narrow only). |

**Preview while image-parked:** if the open revision has no ready primary image and the session is `awaiting_image_ok`, the right pane stays the Instagram preview. The image slot is a dashed soft square (待出圖 / 「圖未出」). Execute (`POST /resume-image`) puts a spinner and 「出緊圖…」 in that same slot. The header is one status name. The footer hides Apply / 確認出街 and points at Discard | 出圖 in chat. Format lock stays on the chat card; the preview does not repeat it. A parked revise keeps the slot with no spinner. See the UI note on [ADR 0004](../adr/0004-stop-discard-and-image-resume.md). Pending copy lands before Execute ([ADR 0036](../adr/0036-image-direction-new-script.md)).

Tenant KB CRUD is **Company settings**, not `/system`. The platform console is the **System** page above.
