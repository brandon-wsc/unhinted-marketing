...
| Company settings | Desktop 1440: shell from UserMenu — shipped sidebar **Voice / Products / Members / Approvals / Models / Instagram**. Frames: Voice; Products Org / Mine; Members; Approvals queue (diff + approve/decline). **Instagram (ADR 0022, drawn):** `empty` (Connect Instagram → Meta OAuth popup) · `connecting` (pending alert; actions hidden) · `connected` (last4, expires, Reconnect / Disconnect) · `expired` (destructive alert + Reconnect). Editor-only. Spec: [knowledge/UI.md](../knowledge/UI.md). |
