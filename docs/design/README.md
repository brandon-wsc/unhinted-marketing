...
| Company settings | Desktop 1440: shell from UserMenu — shipped sidebar **Voice / Products / Members / Approvals / Models**. Frames: Voice; Products Org / Mine; Members; Approvals queue (diff + approve/decline). **Instagram (ADR 0022, drawn):** `empty` (paste IG user id + token + optional expiry) · `connected` (last4, expires, Rotate / Disconnect) · `expired` (destructive alert + Update token). Editor-only; no OAuth this slice. Spec: [knowledge/UI.md](../knowledge/UI.md). |
