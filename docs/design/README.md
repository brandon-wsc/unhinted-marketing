...
**Preview while image-parked:** if the open revision has no ready primary image and the session is `awaiting_image_ok`, the right pane is a 待出圖 board (caption + plan, Apply / 確認出街 disabled) — not the IG mock with a grey image slot. Execute (`POST /resume-image`) shows a spinner. A parked revise keeps the board without that spinner. See the UI note on [ADR 0004](../adr/0004-stop-discard-and-image-resume.md). Pending copy lands before Execute ([ADR 0036](../adr/0036-image-direction-new-script.md)).
