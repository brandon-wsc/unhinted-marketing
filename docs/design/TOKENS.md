...
| `Tabs` | `variant="pills"` (default): track `bg-secondary`; active trigger `bg-accent` + `text-accent-foreground` (voice carries selected); inactive hover `text-foreground` only. `variant="line"`: transparent track + `border-b border-border`; active ink underline (`border-foreground`); hover `bg-secondary`. Products **Org \| Mine** → **line**; settings sidebar nav → default **pills**. |
...
| Session split handle | Structural hairline `bg-border` (1px). Hover affordance is `col-resize` only — do **not** paint `voice-border` / `accent` on the line. Hit is a 7px transparent `::after` (`right: 3px`). Keyboard `focus-visible` uses `ring`. |
