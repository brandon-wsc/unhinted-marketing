...
**Entry:** sign in as `platform_level ≥ 6` → UserMenu → **系統** → SPA `/system` (API is `/api/admin/*`)  
...
4. Freeze a representative case as YAML under [`tests/eval/cases/`](../tests/eval/cases/) (`id`, `suites`, `node`, `state`, `expect`) and re-run:
   ...
   Each case row carries wall `latency_ms`, per-call `ttft_ms` (streaming), `status_counts`, `cached_tokens`, tokens, and rough `usd`; the suite summary shows pass rate, `mean_voice`, p50/p95 (+ ttft), `models_used`, `elapsed_ms`, and `cache_hit_rate`. Compare `reports/eval/latest.md` (gitignored) or `eval_diff` against `reports/eval/baseline.json` (committed) — a `MODEL CHANGED` banner means the run used different models, so metric regressions are informational unless `--strict-models`. Red diff = **fix the node** (or accept a real regression and refresh the anchor via `--out reports/eval/baseline.json` / `eval_diff --accept`) — do not loosen `expect` or rewrite the prompt example just to go green. Follow-up research cases use `queries_require_any` (set coverage), not a Latin-only / no-kana rule. Admin Trace still helps on live sessions; `python -m scripts.eval_cost_from_records` gives p50/p95 + rough $ across prod `llm_call_records`.
