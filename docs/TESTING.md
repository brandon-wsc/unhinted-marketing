...
10. Storage config (ADR 0025): editor GET/PUT `/storage/config` + `/test`; migrate copy → flip → rollback → clean; member 403; secret never in the response (`tests/api/test_company_storage.py`). Live S3 opt-in: `tests/unit/test_media_storage_live.py`.
