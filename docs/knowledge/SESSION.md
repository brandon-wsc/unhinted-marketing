...
- Pick: `POST /sessions/{id}/choose-angle` (`angle_index` or `angle` text, optional `persona` + `image_format`), or a typed `POST /messages` while angle-parked (angle only — persona / format are card fields). A typed message while image-parked is a new script + plan ([ADR 0036](../adr/0036-image-direction-new-script.md)), not 409.
