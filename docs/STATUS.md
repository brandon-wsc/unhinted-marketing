...
| LangGraph graph + interrupt | ✅ | `interrupt_before=["angle_gate", "executor_image_plan"]`; angle pick via `POST /choose-angle` or typed message ([ADR 0028](./adr/0028-angle-pick-before-draft.md)); image resume via `POST /resume-image` ([ADR 0004](./adr/0004-stop-discard-and-image-resume.md)); Stop mid-resume re-parks CTA; Stop while parked discards turn |
