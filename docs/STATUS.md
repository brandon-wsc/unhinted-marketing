...
**Decision (2026-09-08) — Store media object keys; derive URLs at read:** → [ADR 0025](./adr/0025-store-media-object-keys.md). Store-backed `preview_images.url` / `preview_drafts.image_url` hold the object key, not a baked `WEB_BASE_URL` / S3 origin. HTTP, SSE, and Confirm call `resolve_stored_url`. Legacy `/api/media/…` and current-bucket S3 URLs are peeled and re-derived. Provider `http(s)` and `placeholder://` stay as-is.

**Decision (2026-09-08) — Media storage (local on-prem / AWS S3 cloud):** → [ADR 0024](./adr/0024-media-storage-local-and-s3.md). Backend follows `DEPLOYMENT_MODE` (no `STORAGE_BACKEND`, no MinIO / `S3_ENDPOINT_URL`). On-prem writes `MEDIA_ROOT` and serves `GET /api/media/{key}`; cloud writes real AWS S3 (`S3_BUCKET` required at start; keys or instance role). Uploads and `data:` gen results always persist. Object keys include a short token (`r{revision}-{hex}.{ext}`) so concurrent regen/upload cannot overwrite the same bytes; `persist_generated_image` rejects unknown schemes (not `http(s)` / `data:`). §2 URL-in-DB superseded by [ADR 0025](./adr/0025-store-media-object-keys.md).
