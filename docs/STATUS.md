...
| Agent Mode UI (`agent.progress` + cards) | ✅ Done — action trail persisted on user-message metadata; in-flight header `幫緊你幫緊你` / `Working` (no live seconds, chevron stays clickable); done header `做咗x秒` / `Worked for`; image + angle-pick interrupt CTAs survive fail / refresh |
...
- **Instagram Login OAuth (2026-09-06)** — auth-code flow via Instagram dialog (no PKCE, no Facebook Page picker); encrypted connect-state + CSRF on the `social_accounts` row; public `/api/social/oauth/callback` recovers the org from `state`; short-lived token exchanged for 60-day token; `ig_user_id` from `GET /me`. Env: `META_APP_ID` / `META_APP_SECRET` (**Instagram** App ID / Secret), `META_OAUTH_SUCCESS_URL`. **Update (2026-09-20, ADR 0032):** creds now live on `instance_settings` (`meta_app_id` + Fernet `meta_app_secret_last4` via `GET/PUT /api/instance/settings`); env seeds on boot only while `meta_app_id` is unset, then the portal wins. `GET …/oauth/status` reports `configured` + `callback_url` + `mode` for the guided BYO card; **Update (2026-09-20, ADR 0032 §3 built):** `meta_oauth_mode=relay` is live — `meta_oauth_relay_url` (env `OAUTH_RELAY_URL` seeds) + auto-generated `meta_oauth_instance_id` on `instance_settings`; browser start hits `{relay}/authorize`, the Cloudflare Worker (`relay/`) exchanges the code against a REGISTRY-verified instance slug and 302s to `/api/social/oauth/relay-finish?ticket&state`, which redeems a one-time 60s KV ticket server-to-server. Callback is `{web_base_url}/api/social/oauth/callback` (no `META_OAUTH_REDIRECT_URI`). Scopes: `instagram_business_basic`, `instagram_business_content_publish`. Failures: `meta_oauth_not_professional`, `meta_oauth_missing_publish`. CLI/PUT seeding still available as fallback. Abandoned connect: 10-minute poll timeout + Cancel (`POST …/oauth/cancel`). Publish host: `graph.instagram.com`. Relay URL is a fixed product endpoint (`HOSTED_OAUTH_RELAY_URL`, hosted Unhinted relay) — read-only: `PUT /api/instance/settings` does not accept it and `OAUTH_RELAY_URL` is authoritative on every boot, so self-hosted/dev relays can switch (or switch back) via env alone. **Update (2026-09-20, [ADR 0034](./adr/0034-relay-per-install-shared-secret.md)):** REGISTRY entries are `{url, secret}` JSON — a per-install shared secret issued at registration (paste into System → Instance, Fernet at rest; env `OAUTH_RELAY_SECRET` seeds dev/UAT). Ticket redeem requires `sig=HMAC(secret, "ticket:{id}")` and relayed platform events are HMAC'd with the secret, so the public `instance_id` slug alone proves nothing; registry URLs must be `https:` (loopback excepted), and data-deletion answers 503 instead of a false "completed" when a known install is unreachable.
- **Meta Live-mode platform callbacks (2026-09-20, [ADR 0033](./adr/0033-meta-live-mode-platform-callbacks.md))** — `POST /api/social/meta/deauthorize` + `POST /api/social/meta/data-deletion` verify Meta's `signed_request` (HMAC-SHA256, app secret) and drop `social_accounts` rows by `ig_user_id`; data deletion answers `{url, confirmation_code}` with a stateless HMAC code + `GET …/data-deletion/{code}` status. Relay mode forwards verified events to the owning install via `POST /api/social/meta/relay` (HMAC keyed on `meta_oauth_instance_id`); the Worker keeps a `u:{ig_user_id}` → instance map (90d) and exposes the fixed vendor URLs `/meta/deauthorize` + `/meta/data-deletion`. `oauth/status` + `GET /api/instance/settings` expose all three dashboard URLs for copy-paste.
...
**Decision (2026-09-20) — On-prem Meta OAuth: BYO-app default + opt-in relay:** → [ADR 0032](./adr/0032-meta-oauth-byo-and-relay.md). Meta Strict Mode exact-match makes a vendor app unserveable on customer domains, so on-prem uses its own Meta app held in `instance_settings` (`meta_app_id` + Fernet `meta_app_secret`, env seeds then portal wins) with guided setup in the Instagram panel (own-brand = Standard Access, no App Review). The opt-in `meta_oauth_mode=relay` ships in the same slice — a Cloudflare Worker (`relay/`) does the exchange vendor-side and fans back out over one-time 60s tickets to REGISTRY-registered destinations only, never a silent default. **Hardened by [ADR 0034](./adr/0034-relay-per-install-shared-secret.md):** per-install shared secret on REGISTRY entries authenticates ticket redemption + relayed events (public slug proves nothing).

**Decision (2026-09-19) — Image format rides the bundled angle gate:** → [ADR 0030](./adr/0030-image-format-in-bundled-gate.md) (extends [ADR 0029](./adr/0029-angle-persona-bundled-gate.md); supersedes its §4 park condition; §6 late switch superseded by [ADR 0036](./adr/0036-image-direction-new-script.md)). The single-vs-comic pick moves from the post-draft image park onto the bundled card: one `POST /choose-angle` submit answers angle + persona + `image_format`, so `executor_post` drafts with the format known (comic captions complement a 4-panel arc). Typed reply while parked stays angle-only; the pick is sticky across re-brainstorm cycles. The gate now parks whenever ≥1 angle is offered — a lone angle still needs confirm and the format question is always asked; only a 0-angle brief drafts immediately. A later format change is a new script, not an image-only redraw.

**Decision (2026-09-19) — Bundled angle + persona gate:** → [ADR 0029](./adr/0029-angle-persona-bundled-gate.md) (supersedes [ADR 0028](./adr/0028-angle-pick-before-draft.md); park condition superseded by [ADR 0030](./adr/0030-image-format-in-bundled-gate.md) §7). Same single park after `brainstormer` when `brief.angles` ≥ 1; the card now submits angle **and** persona in one `POST /choose-angle`. Typed reply still resolves angle only (persona via the card field). Soft chat clarify is unchanged; no LLM-initiated structured park.

**Decision (2026-09-15) — Angle pick parks before drafting:** → [ADR 0028](./adr/0028-angle-pick-before-draft.md) (superseded by [ADR 0029](./adr/0029-angle-persona-bundled-gate.md); park `≥ 2` superseded by [ADR 0030](./adr/0030-image-format-in-bundled-gate.md) §7). `brainstormer → executor_post` no longer runs unattended: `brief.angles` ≥ 1 parks the graph at `angle_gate`; user picks via `POST /sessions/{id}/choose-angle` card or a typed reply (the only park where a message resumes). Non-matching text re-briefs. Stop/discard unchanged (ADR 0004).
...
**Decision (2026-09-22) — Direction change is a new script, then Discard or Execute:** → [ADR 0036](./adr/0036-image-direction-new-script.md). Supersedes ADR 0030 §6 and ADR 0031. Changing the picture (chat 「唔要／改成」 or an edit-image prompt/format/plan change) writes a new caption and image plan onto a preview revision **before** Execute, and parks before `executor_image_gen`. That version is only Discard (restores the last accepted revision; `POST /stop` returns that preview so the pane paints caption, image, and revision) or Execute (`POST /resume-image` draws the locked plan). No format chips; a different `image_format` on resume is 400. `POST .../media/{id}/regen` only re-samples the current locked plan and refuses while a version is pending. Chat Send while image-parked is that regret cycle, not a queue. A caption-only Send updates the pending caption, keeps the locked plan, and stays parked; it does not accept the version.

**Decision (2026-09-19) — Queue Send while parked at image OK:** → [ADR 0031](./adr/0031-queue-send-while-image-parked.md) (superseded by [ADR 0036](./adr/0036-image-direction-new-script.md)). Angle-park Send stays a typed pick. A format change rewrites the plan **and** the script before Execute so a comic→single switch is not a stamped format on an old 4-panel prompt or an old caption.
...
- **Buy the inner loop, keep the graph** — node-internal LLM calls that need a tool loop (chat / research / execute) adopt Pydantic AI (narrow per-stage agents, `output_type` on `schemas/` models, LiteLLM BYOK unchanged); LangGraph is narrowed but **kept** for mode routing, checkpointer, interrupts (`angle_gate` / image), turn lifecycle. Grounding / reviewer stay single-shot
...
| Automated tests | ✅ Backend + FE Tier 1/2 + CI + on-demand live eval | BE: `tests/unit` + `tests/api` (`TEST_DATABASE_URL`). FE: `cd web && pnpm test` (Vitest + RTL — lib utils + `session-helpers` / `session-layout` / `useSession` + PasswordBox / UserMenuDropdown). Live LLM: `python -m scripts.eval_agent` (not CI) — per-case latency/ttft/tokens/cache/usd + status counts, suite `models_used`/`elapsed_ms`, committed `reports/eval/baseline.json` + `scripts.eval_diff` regression check (MODEL CHANGED downgrades metric deltas to warnings; `--strict-models`), 35-case pack incl. grounding + safety fixtures; deterministic cases run keyless; prod cost surface `scripts.eval_cost_from_records`. Policy [TESTING.md](./TESTING.md). CI: [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) |
...
| Item | Status | Notes |
|------|--------|-------|
| ROADMAP graph contract | ✅ | 13 nodes + `persist_preview`; edges + two interrupts |
...
| LangGraph graph + interrupt | ✅ | `interrupt_before=["angle_gate", "executor_image_gen"]` ([ADR 0036](./adr/0036-image-direction-new-script.md)); angle pick via `POST /choose-angle` or typed message ([ADR 0028](./adr/0028-angle-pick-before-draft.md)); image resume via `POST /resume-image` ([ADR 0004](./adr/0004-stop-discard-and-image-resume.md)); Stop mid-resume re-parks CTA; Stop while parked discards the pending revision |
...
| Agent Mode UI | ✅ | `agent.progress` live inside each graph node; Cursor-style action-record trail persisted on the triggering user row as `session_messages.metadata.agent_actions` (hydrate on reopen) plus turn `duration_ms`. In-flight header is `幫緊你幫緊你` / `Working` (no ticking seconds; chevron stays enabled). Completed header is `做咗x秒` / `Worked for`. Node labels: running present-tense + `…`, done past tense. Optimistic `route_intent` while POST/SSE catch up; hide `fast_rule_checker` / `persist_preview`; brief card + interrupt cards (`draft.awaiting_image_ok` / `draft.awaiting_angle_pick` / snapshot `interrupted`); resume image via `POST /resume-image`; angle pick via card or typed reply ([ADR 0028](./adr/0028-angle-pick-before-draft.md)); composer open while in-flight with FE send queue (max 3, holds at either park) and Stop beside Send ([ADR 0016](./adr/0016-queue-send-while-turn-in-flight.md)); Stop mid-image / mid choose-angle re-parks the CTA; Stop while parked discards turn ([ADR 0004](./adr/0004-stop-discard-and-image-resume.md)) |
...
- **Sessions:** `GET /api/sessions`, `POST /api/sessions`, `PATCH /api/sessions/{id}` (title / pinned), `DELETE /api/sessions/{id}`, `POST /api/sessions/{id}/messages`, `GET /api/sessions/{id}/messages`, `POST /api/sessions/{id}/choose-angle`, `POST /api/sessions/{id}/resume-image`, `POST /api/sessions/{id}/stop`, `POST /api/sessions/{id}/draft`, `GET/POST /api/sessions/{id}/media`, `PATCH /api/sessions/{id}/media/{image_id}/plan`, `POST /api/sessions/{id}/media/{image_id}/regen`, `POST /api/sessions/{id}/media/{image_id}/remove`, `POST /api/sessions/{id}/media/{image_id}/upload`, `GET /api/sessions/{id}/events` (SSE), `POST /api/sessions/{id}/confirm`
...
| `META_APP_ID` / `META_APP_SECRET` | Seed-only (ADR 0032): Instagram App ID / Secret stored Fernet-encrypted on `instance_settings`; afterwards System → Instance wins. Whitelist `{WEB_BASE_URL}/api/social/oauth/callback` as the Valid OAuth Redirect URI |
| `META_OAUTH_SUCCESS_URL` | Optional post-connect SPA URL (defaults to `WEB_BASE_URL` + `/settings?tab=instagram`) |
| `OAUTH_RELAY_URL` | Optional Meta OAuth relay base (ADR 0032 §3): defaults to the hosted `HOSTED_OAUTH_RELAY_URL`; set only for a self-hosted/dev relay. Authoritative on every boot — `meta_oauth_relay_url` is read-only in System → Instance |
| `OAUTH_RELAY_SECRET` | Optional relay shared-secret seed (ADR 0034): the secret the relay issues at REGISTRY registration. Seeds `instance_settings` Fernet-encrypted only while unset; afterwards the portal value wins. Dev/UAT convenience — production pastes via System → Instance |
| `META_OAUTH_INSTANCE_ID` | Optional pin for the relay REGISTRY slug (`meta_oauth_instance_id`); auto-generated on seed otherwise. Dev/UAT only — production installs let it generate |
...
| **H2** | Interrupt resume ignores user intent (`ainvoke(None)`) | ✅ **Mitigated** — [ADR 0004](./adr/0004-stop-discard-and-image-resume.md) + [ADR 0016](./adr/0016-queue-send-while-turn-in-flight.md) + [ADR 0028](./adr/0028-angle-pick-before-draft.md): Stop discards parked turns; FE queue while in-flight and at either park; image resume `POST /resume-image` only; angle pick via `POST /choose-angle` or typed `/messages`; `/messages` 409 while busy or image-parked; chat/JSON completions stream + `aclose` on cancel (best-effort upstream abort) |
