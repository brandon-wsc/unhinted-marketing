...
5. **Stop / cancel reuse ADR 0004:** parked discard deletes the graph thread + restores pre-turn `session.state`; Stop mid `POST /choose-angle` re-parks at `angle_gate` (turn registry `kind="choose_angle"` + parked snapshot, mirroring `resume_image`).
