...
3. **Backend participates:** Per-session in-memory turn registry holds the running `asyncio.Task`. `POST /sessions/{id}/stop` cancels the task (in-flight) or discards parked state. Concurrent `POST /messages` while busy or **image-parked** returns **409**. Parked at `angle_gate` is the exception — a typed message is a pick ([ADR 0028](./0028-angle-pick-before-draft.md)).
4. **Image resume is explicit:** Only `POST /sessions/{id}/resume-image` may `ainvoke(None)` when parked at `executor_image_plan`. Normal `POST /messages` never blind-resumes the image interrupt.
