...
- **The API surface is already BYOK-friendly for Completions clones.** `openai` / `openai_compatible` / (Anthropic Messages aside) run on Chat Completions + Images API (no Responses API). Chat Completions is `{base}/chat/completions` for every clone. Images are **shape-detected**: `GET {base}/images/models` 200 JSON → `{base}/images`; otherwise LiteLLM `{base}/images/generations` (OpenAI DALL·E). `litellm.drop_params = True` silently drops our two OpenAI-specific assumptions (`response_format: json_object`, `stream_options.include_usage`) for stricter providers — **that flag is the compatibility backstop and must not be removed**. Native Gemini and Vertex Express are exceptions: see [ADR 0021](./0021-org-byok-native-gemini.md).
