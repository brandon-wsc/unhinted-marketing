...
- **Amended by:** [ADR 0021](./0021-org-byok-native-gemini.md) (`provider_type` adds `gemini` and `vertex_ai`; Completions + Images is the surface for `openai` / `anthropic` / `openai_compatible` only). **Compat image path (2026-09-12):** `openai_compatible` (and any custom `api_base`) does **not** always speak OpenAI `POST /v1/images/generations`. When `GET {api_base}/images/models` returns JSON, image gen uses `POST {api_base}/images` with `{model, prompt}` and the catalog id (no LiteLLM `openrouter/` / `openai/` prefix; no DALL·E `size=1024x1024` — catalog size is `1K`/`2K`/`4K` or omit). 404 / error keeps LiteLLM `/images/generations`. Clones stay `openai_compatible` — no `openrouter` provider type. Model registry / env ids are catalog slugs (`bytedance-seed/seedream-4.5`). Org `api_base` still uses the SSRF helper; env `LLM_API_BASE` is operator-trusted.
...
- **The API surface is already BYOK-friendly for Completions clones.** `openai` / `openai_compatible` / (Anthropic Messages aside) run on Chat Completions + Images API (no Responses API). Chat Completions is `{base}/chat/completions` for every clone. Images are **shape-detected**: `GET {base}/images/models` 200 JSON → `{base}/images` with `{model, prompt}` (no DALL·E `size`); otherwise LiteLLM `{base}/images/generations` (OpenAI DALL·E). `litellm.drop_params = True` silently drops our two OpenAI-specific assumptions (`response_format: json_object`, `stream_options.include_usage`) for stricter providers — **that flag is the compatibility backstop and must not be removed**. Native Gemini and Vertex Express are exceptions: see [ADR 0021](./0021-org-byok-native-gemini.md).
