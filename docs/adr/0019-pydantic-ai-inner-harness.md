...
- Research ingest remains PostgreSQL persist; the `query_generator` agent only calls the existing Tavily∪PG adapter (`ingest_web_search`). `research_ingest` still merges PG and sets `signals_trusted`; it skips a second Tavily fetch when the agent already ingested this turn.
- The `executor_post` agent only emits typed `DraftOut`; publish / Confirm stay HTTP and are never tools. Citation allowlisting still happens in the node after the agent returns.
- The `edit_copy` agent only emits typed `EditOut`; publish / Confirm stay HTTP and are never tools. Citation allowlisting and `need_image` still happen in the node after the agent returns.
