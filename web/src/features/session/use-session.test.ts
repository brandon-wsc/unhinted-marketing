...
describe("useSession", () => {
  ...
  it("stopTurn discards parked turn and reloads messages", async () => {
    ...
    expect(apiStopSessionTurn).toHaveBeenCalledWith("tok", "sess-1", "discard");
    expect(result.current.awaitingImageOk).toBe(false);
    expect(result.current.messages).toEqual([]);
    expect(result.current.composerLocked).toBe(false);
  });
  ...
  it("drains the queue head after stopTurn discards an in-flight turn", async () => {
    const priorAssistant = {
      id: "a1",
      session_id: "sess-1",
      role: "assistant",
    ...
    act(() => {
      void result.current.sendMessage("先出一版");
    });
    await waitFor(() => expect(result.current.sending).toBe(true));

    act(() => {
      expect(result.current.enqueueQueuedMessage("跟住改 caption")).toBe(true);
    });
  ...
  it("interrupt stop keeps the in-flight turn's messages, then drains", async () => {
    const priorAssistant = {
      id: "a1",
      session_id: "sess-1",
      role: "assistant",
    ...
    apiGetSessionMessages
      ...
      .mockResolvedValue({
        session: sessionFixture,
        messages: [priorAssistant, inFlightUser, partialAssistant],
      });
    ...
    await act(async () => {
      await result.current.stopTurn("interrupt");
    });

    expect(apiStopSessionTurn).toHaveBeenCalledWith("tok", "sess-1", "interrupt");
    // Interrupted turn's rows stay — the queued message appends, not replaces.
    await waitFor(() =>
      expect(apiPostSessionMessage).toHaveBeenCalledWith(
        "tok",
        "sess-1",
    ...
    expect(result.current.messages.map((m) => m.id)).toEqual(
      expect.arrayContaining(["a1", "u1", "a2"]),
    );
    expect(result.current.queuedMessages).toHaveLength(0);
  });

  it("turn.cancelled with kept:true does not prune the turn's messages", async () => {
    getRememberedSessionId.mockReturnValue("sess-1");
    apiGetSessionMessages.mockResolvedValue({
      session: sessionFixture,
      messages: [msgA],
    ...
    apiPostSessionMessage.mockReturnValue(new Promise(() => {}));
    ...
    expect(result.current.messages.some((m) => m.content === "先出一版")).toBe(true);
  });
